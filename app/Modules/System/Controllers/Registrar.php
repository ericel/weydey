...
class Registrar extends BackendController
{
    ...
    {
        return $this->getView()
            ->shares('title', __d('system', 'Register a free account'))
            ->shares('description', __('Authenticated users can share and downloads files for free.'));
    }
    ...
    public function checkUsername(){
        if(isset($_POST["usernamecheck"])){
          ...
          $usernameCheckArray = User::where('username', '=', $username)->get();
          if(count($usernameCheckArray) > 0){
            echo '<div class="red-text"><i class="fa fa-exclamation-circle" aria-hidden="true"></i> Sorry '.$username.' is not available!</div>';
            exit();
          } else {
            echo '<div class="green-text"><i class="fa fa-check-circle" aria-hidden="true"></i> '.$username.' is okay!</div>';
            exit();
          }
        }
    }
    public function store()
    {
        ...
        Mailer::send('Emails/Auth/Activate', array('token' => $token, 'username' => $input['username']), function($message) use ($user)
        {
